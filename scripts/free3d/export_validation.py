...
def _texture_image(document: dict[str, Any], binary: bytes, reference: Any) -> tuple[bytes, int] | None:
    ...
    if not isinstance(length, int) or start < 0 or length <= 0 or start + length > len(binary):
        return None
...
def inspect_glb_surface(path: Path) -> dict[str, Any]:
    ...
    base_image = _texture_image(document, binary, base_reference)
    roughness_image = _texture_image(document, binary, roughness_reference)
    base_color_texture = base_image is not None
    roughness_texture = roughness_image is not None
    repacked_roughness = bool(base_image and roughness_image and base_image[1] != roughness_image[1])
    ...
    if base_color_texture and roughness_texture and not repacked_roughness:
